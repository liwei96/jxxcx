<template>
	<view class="piclist">
		...
		<view class="swiperMain">
			<view class="nav_nav">
				<scroll-view class="nav_list" scroll-x="true">
					<view :class="num==index ? 'active':'nav_one' " v-for="(item,index) in swiperTabList" :key="index"
						@tap="clickOne(index)">{{item }}</view>
				</scroll-view>
		...
		<bottom :remark="'楼盘相册页+预约看房'" :point="103" :title="'预约看房'" :pid="parseInt(project_id)" :telphone="telphone">
		</bottom>
	...
	export default {
		...
		methods: {
			getinfo(id) {
				...
				uni.request({
					url: this.javaserve + "/applets/jy/building/images",
					data: {
						id: id,
						other: other,
						token: token,
					...
					success: (res) => {
						...
						if (res.data.status == 200) {
							...
							this.traffic = img.traffic;
							this.effect = img.effect;
							this.real = img.real;
							this.example = img.sample;
							this.matching = img.matching;
							this.departments = img.house_type;
							...
							this.swiperTabList[2] = `样板图(${img.sample.length})`;
							this.swiperTabList[3] = `交通图(${img.traffic.length})`;
							this.swiperTabList[4] = `户型图(${img.house_type.length})`;
							this.swiperTabList[5] = `配套图(${img.matching.length})`;
							// #ifdef MP-BAIDU
							let header = res.data.data.header;
							let my_img = res.data.data.images.effect;
							let arr = [];
							my_img.map(p => {
								arr.push(p.small)
							})
